...
func (prov *ProvisioningAPI) Init() {
	...
	prov.bridge.AS.Router.HandleFunc(
		"/_matrix/app/com.beeper.asmux/ping", prov.BridgeStatePing,
	).Methods(http.MethodPost)
	prov.bridge.AS.Router.HandleFunc(
		"/_matrix/app/com.beeper.bridge_state", prov.BridgeStatePing,
	).Methods(http.MethodPost)
	...
	return http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			if auth != prov.bridge.Config.Bridge.Provisioning.SharedSecret {
				...
				jsonResponse(
					w, http.StatusForbidden, map[string]interface{}{
			...
			prov.log.Infofln(
				"%s %s from %s took %.2f seconds and returned status %d", r.Method, r.URL.Path, user.MXID, duration,
				wWrap.statusCode,
			)
		},
	)
...
func (prov *ProvisioningAPI) OpenGroup(w http.ResponseWriter, r *http.Request) {
	...
	} else if jid, err := types.ParseJID(groupID); err != nil || jid.Server != types.GroupServer || (!strings.ContainsRune(
		jid.User, '-',
	) && len(jid.User) < 15) {
		jsonResponse(
			w, http.StatusBadRequest, Error{
